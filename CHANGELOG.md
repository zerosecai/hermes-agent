...
- **`PRIVACY.md`** — explicit, verifiable privacy posture: no third-party analytics, no phone-home, no exfiltration. Names every `telemetry` identifier in the codebase, explains it's local-only, and binds future contributors to opt-in defaults for any third-party data flow.

### Changed
- **`README.md`** — rewritten with Kuma identity, honest "Day-1 status" disclosure that the CLI command, Python module names, and documentation URLs still use `hermes`. Links upstream Hermes docs as authoritative until Kuma has its own docs site. Banner image at the top now shows the Kuma cyber-bear instead of the original Hermes graphic. Privacy-first bullet now cites the verifiable `PRIVACY.md`.
...
- **`SECURITY.md`** — title and intro retitled; vulnerability reporting routes Kuma-specific issues to `zerosecai` GHSA / `sam@zerosec-ai.com`, upstream-runtime issues stay with Nous (with a CC ask). New "Privacy Posture" section (§5) summarises the no-telemetry guarantee and links to `PRIVACY.md`. All technical content (trust model, sandbox, redaction, subagent isolation) is upstream-equivalent and untouched.
